            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 rounded-2xl border-2 border-white/20 overflow-hidden shadow-2xl",
                    "will-change-transform select-none",
                    isActive
                      ? "cursor-grab active:cursor-grabbing ring-2 ring-primary/40"
                      : "cursor-pointer",
                  )}
                  style={{
                    width: cardWidth,
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    on ? "bg-primary w-6" : "bg-zinc-300 hover:bg-zinc-400",
                  )}
    ...
    <div className="relative h-full w-full">
      ...
      {item.tag ? (
        <div className="absolute top-3 right-3 z-10">
          <span className="inline-block rounded-full bg-primary/90 px-3 py-1 text-xs font-semibold text-white backdrop-blur-xs shadow-md">
            {item.tag}
          </span>
        </div>
      ) : null}
      ...
      <div className="relative z-10 flex h-full flex-col justify-end p-5">
        ...
        {item.description ? (
          <div className="mt-1 line-clamp-2 text-xs sm:text-sm text-zinc-200">
            {item.description}
          </div>
        ) : null}
      </div>
